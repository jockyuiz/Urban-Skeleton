...
namespace SD_Evaluation
{
    ...
    public static class MathUtil
    {
        ...
        public static void RemapList(List<double> origin, double min, double max, double lower_thred, double upper_thred)
        {
            double ref_max = origin.Max();
            double ref_min = origin.Min();

            for(int i = 0; i < origin.Count; ++i)
            {
                double value = origin[i];
                if (value >= upper_thred) origin[i] = max;
                else if (value <= lower_thred) origin[i] = min;
                else origin[i] = Map(value, ref_min, ref_max, min, max);
            }
        }
